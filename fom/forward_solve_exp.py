...
class Fin:
    ...
    def __init__(self, V, external_obs=False):
        ...
        self._adj_F = inner(exp(self._k) * grad(self.w_hat), grad(self.v_trial)) * self.dx(0)\
    ...
    def gradient(self, k, data):
        ...
        grad_w_form = k_hat * exp(self._k) * inner(grad(z), grad(v)) * self.dx(0)
    ...
    def sensitivity(self, k):
        ...
        #  grad_w_form = k_hat * exp(self._k) * inner(grad(z), grad(v)) * self.dx(0)
        grad_w_form = k_hat * exp(self._k) * inner(grad(z), grad(v_hat)) * self.dx(0)
        #  grad_vec = assemble(grad_w_form)[:]
